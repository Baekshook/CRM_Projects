    ...
    const loadData = async () => {
      try {
        ...
        const [negotiationsData, customersData, singersData] =
          await Promise.all([
            getNegotiationsByStatus("final-quote"),
            getAllCustomers(),
            getAllSingers(),
          ]);
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-4 py-2 border-b border-gray-200 bg-gray-50">
          <h2 className="text-sm font-medium text-black">
            총 {filteredNegotiations.length}개의 확정 견적서
          </h2>
        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex space-x-2 justify-end">
                          ...
                          <button
                            ...
                            onClick={() => handleViewQuote(negotiation.id)}
                          >
                            견적서
                          </button>
                          <button
                            className="text-purple-600 hover:text-purple-900"
                            onClick={() => handlePrintQuote(negotiation.id)}
                          >
                            인쇄
                          </button>
                          <button
                            className="text-orange-600 hover:text-orange-900"
                            onClick={() => handleCreateContract(negotiation.id)}
                          >
                            계약서
                          </button>
                        </div>
