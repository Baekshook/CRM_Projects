...
export default function InProgressRequestsPage() {
    ...
    const loadData = async () => {
      try {
        ...
        const [requestsData, customersData, singersData] = await Promise.all([
          getRequestsByStatus("in_progress"),
          getAllCustomers(),
          getAllSingers(),
        ]);
  ...
  return (
    <div className="pb-10">
      <PageHeader title="진행 중인 요청서" />

      <div className="mb-6">
        <h2 className="text-2xl font-bold text-black">진행 중인 요청서 관리</h2>
        <p className="text-black mt-1">
          현재 진행 중인 모든 요청서를 확인하고 관리하세요.
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-4 py-2 border-b border-gray-200 bg-gray-50">
          <h2 className="text-sm font-medium text-black">
            총 {filteredRequests.length}개의 진행 중인 요청서
          </h2>
        </div>
