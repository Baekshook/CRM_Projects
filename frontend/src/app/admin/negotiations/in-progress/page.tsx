...
export default function InProgressNegotiationsPage() {
  ...
  // 협상 중(negotiating) 상태의 매칭만 필터링
  const negotiatingMatches = matches.filter(
    (match) => match.status === "negotiating"
  );

  // 필터링된 매칭 목록
  const filteredMatches = negotiatingMatches.filter((match) => {
    // 고객 필터
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-4 py-2 border-b border-gray-200 bg-gray-50">
          <h2 className="text-sm font-medium text-black">
            총 {filteredMatches.length}개의 협상 진행 중
          </h2>
        </div>
